...
then
    ...
    /bin/bash -l -c 'chown -R app:app /home/app/webapp/public/system' # mounted volume may have wrong permissions
    /sbin/setuser app /bin/bash -l -c 'cd /home/app/webapp && yarn install && bundle exec rake db:migrate && bundle exec whenever --update-crontab'
