...
migrations_run=`PGPASSWORD=$POSTGRES_PASSWORD psql -h $POSTGRES_HOST -U $POSTGRES_USER $POSTGRES_NAME -t -c "SELECT version FROM schema_migrations ORDER BY schema_migrations" | wc -c`
